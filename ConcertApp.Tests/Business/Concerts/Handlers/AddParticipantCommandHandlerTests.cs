...
namespace ConcertApp.Tests.Business.Users.Handlers
{
    ...
    public class AddParticipantCommandHandlerTests
    {
        ...
        private AddParticipantCommandHandler _handler;
        private AddParticipantCommand _request;

        [SetUp]
        public void Init()
        {
            _context = new Mock<ConcertAppContext>();
            _handler = new AddParticipantCommandHandler(_context.Object);
        ...
        public async Task ShouldAddParticipantAsync()
        {
            ...
            _context.Verify(c => c.UserConcerts
                .AddAsync(It.IsAny<UserConcert>(), It.IsAny<CancellationToken>()), Times.Once);
            _context.Verify(c => c.UserConcerts
                .AddAsync(It.Is<UserConcert>(obj =>
                    obj.ConcertId == _request.ConcertId &&
                    obj.UserId == 2 &&
                    obj.UserStatus == UserStatus.Participant), It.IsAny<CancellationToken>()), Times.Once);
        }
        ...
        [Test]
        public Task WhenConcertCapacityReached_ShouldReturnError()
        {
            _request.ConcertId = 2;
            var expectedResult = JsonSerializer.Serialize(new
            {
                ErrorCode = (int)ErrorCodes.Concert_CapacityReached,
                Message = ConcertErrors.CapacityReached
            });
            Func<Task> action = async () => await _handler.Handle(_request, new CancellationToken());
            CustomException ex = Assert.ThrowsAsync<CustomException>(async () => await action());
        ...
        [Test]
        public Task WhenUserIsParticipating_ShouldReturnError()
        {
            _request.ConcertId = 4;
            var expectedResult = JsonSerializer.Serialize(new
            {
                ErrorCode = (int)ErrorCodes.Concert_AlreadyParticipating,
                Message = ConcertErrors.UserAlreadyParticipating
            });
            Func<Task> action = async () => await _handler.Handle(_request, new CancellationToken());
            CustomException ex = Assert.ThrowsAsync<CustomException>(async () => await action());
        ...
        {
            _request = new AddParticipantCommand
            {
                Email = "guy@gmail.com",
                ConcertId = 1,
            };
